...
if __name__ == '__main__':
    ...
    for strategy in results:
        ...
        plt.savefig('save_figs/offshore_{}_{}.pdf'.format(strategy, tp))

        os.makedirs('results/offshore_EoL', exist_ok=True)
        # export plot data as df
        df = pd.DataFrame(result_sum, columns=proc_methods, index=year)
        df.to_csv('results/offshore_EoL/offshore_{}_sum_{}.csv'.format(strategy, tp))

        for m in result.keys():
            df = pd.DataFrame(result[m], columns=proc_methods, index=year)
            df.to_csv('results/offshore_EoL/offshore_{}_{}_{}.csv'.format(strategy, m, tp))